...
async function validateToken(tokenAddress) {
  try {
    // Use the proper token validator (same as bot uses)
    const tokenInfo = await tokenValidator.validateToken(tokenAddress);
    ...
    return {
      symbol: tokenInfo.marketData?.symbol || 'Unknown',
      name: tokenInfo.marketData?.symbol || 'Unknown',
      decimals: tokenInfo.decimals,
      priceUsd: tokenInfo.marketData?.price || 0,
      liquidity: tokenInfo.marketData?.liquidity || 0,
      volume24h: tokenInfo.marketData?.volume24h || 0,
      jupiterSupported: tokenInfo.jupiterSupported,
